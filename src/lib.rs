...
#[derive(Debug, Clone)]
pub struct APlusBSqrtN(APlusBSqrtN_);
...
impl APlusBSqrtN {
    ...
    pub fn new(a: BigFraction, b: BigFraction, n: BigUint) -> Self {
        ...
        if b == BigFraction::zero() || is_perfect_square(&n) {
            Self(APlusBSqrtN_::Rational(a + b * n.sqrt()))
        } else {
            Self(APlusBSqrtN_::Irrational { a, b, n })
        }
    }
    ...
    pub fn from_rational(a: BigFraction) -> Self {
        ...
        Self(APlusBSqrtN_::Rational(a))
    }

    #[must_use]
    pub const fn is_rational(&self) -> bool {
        matches!(self, APlusBSqrtN(APlusBSqrtN_::Rational(_)))
    }
}
...
impl PartialEq for APlusBSqrtN {
    fn eq(&self, other: &Self) -> bool {
        match (&self.0, &other.0) {
            (APlusBSqrtN_::Rational(a1), APlusBSqrtN_::Rational(a2)) => a1 == a2,
            (APlusBSqrtN_::Rational(_), APlusBSqrtN_::Irrational { .. })
            | (APlusBSqrtN_::Irrational { .. }, APlusBSqrtN_::Rational(_)) => false,
            (
                APlusBSqrtN_::Irrational {
                    a: a1,
                    b: b1,
                    n: n1,
                },
...
impl ops::Neg for APlusBSqrtN {
    ...
    fn neg(self) -> Self::Output {
        match self.0 {
            APlusBSqrtN_::Rational(a) => Self(APlusBSqrtN_::Rational(-a)),
            APlusBSqrtN_::Irrational { a, b, n } => Self::new(-a, -b, n),
        }
    }
}
...
impl ops::Add for APlusBSqrtN {
    ...
    fn add(self, other: Self) -> Self::Output {
        match (self.0, other.0) {
            (APlusBSqrtN_::Rational(a1), APlusBSqrtN_::Rational(a2)) => {
                Self(APlusBSqrtN_::Rational(a1 + a2))
            }
            (APlusBSqrtN_::Irrational { a, b, n }, APlusBSqrtN_::Rational(a2))
            | (APlusBSqrtN_::Rational(a2), APlusBSqrtN_::Irrational { a, b, n }) => {
                Self::new(a + a2, b, n)
            }
...
impl ops::Mul for APlusBSqrtN {
    ...
    fn mul(self, rhs: Self) -> Self::Output {
        match (self.0, rhs.0) {
            (APlusBSqrtN_::Rational(a1), APlusBSqrtN_::Rational(a2)) => Self::from_rational(a1 * a2),
            (APlusBSqrtN_::Rational(multiplier), APlusBSqrtN_::Irrational { a, b, n })
            | (APlusBSqrtN_::Irrational { a, b, n }, APlusBSqrtN_::Rational(multiplier)) => {
...
impl APlusBSqrtN {
    ...
    pub fn recip(self) -> Self {
        match self.0 {
            APlusBSqrtN_::Rational(a) => Self::from_rational(GenericFraction::one() / a), /* has to check that this does not lead to infinity */
            APlusBSqrtN_::Irrational { a, b, n } => {
                // ans = 1 / [a + b * sqrt(n)]
...
impl APlusBSqrtN {
    ...
    pub fn is_positive(&self) -> bool {
        match &self.0 {
            APlusBSqrtN_::Rational(a) => a > &BigFraction::zero(),
            APlusBSqrtN_::Irrational { a, b, n } => {
...
impl APlusBSqrtN {
    ...
    pub fn floor(self) -> GenericFraction<BigUint> {
        match &self.0 {
            APlusBSqrtN_::Rational(a) => a.floor(),
            APlusBSqrtN_::Irrational { a, b, n } => {
                ...
                if Self::from_rational(m.clone() + GenericFraction::one()) <= self.clone() {
                    if self.clone()
                        >= Self::from_rational(
                ...
                } else if Self::from_rational(m.clone()) <= self.clone() {
                    return m;
                } else if Self::from_rational(m.clone() - GenericFraction::one()) <= self.clone() {
                    return m - GenericFraction::one();
                }
...
impl num_traits::identities::Zero for APlusBSqrtN {
    fn zero() -> Self {
        Self(APlusBSqrtN_::Rational(GenericFraction::zero()))
    }
...
impl num_traits::identities::One for APlusBSqrtN {
    fn one() -> Self {
        Self(APlusBSqrtN_::Rational(GenericFraction::one()))
    }
}
